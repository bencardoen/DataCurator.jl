...
N = create_template_from_toml("example_recipes/verifier.toml")
c, t = N
t
cs, ls, x = delegate(c, t)
...
# template[4] = [mt(inputdir_check, onfail), mt(x->~inputdir_check(x), on_input_dir)]
#
# file_check = x -> is_3d_img(x) & endswith(x, r"[1,2].tif")
# template[5] = [mt(x->~file_check(x), onsuccess),mt(file_check, onfail)]
#
#
